...
const SEED_MEMBERS = [
  { id:"m001", name:"Aisha Rahman", phone:"012-3456-789", points:3200, referredBy:null,   joinedAt:"2024-01-10", referralCode:"AISHA-2024",
  ...
  { id:"m002", name:"Daniel Tan",   phone:"016-8877-001", points:720,  referredBy:"m001", joinedAt:"2024-02-14", referralCode:"DTAN-5528",
    transactions:[{id:"t2a",pts:720,icon:"⭐",label:"Welcome + First Purchase",date:"Feb 14",type:"earn"}]},
  { id:"m003", name:"Priya Nair",   phone:"011-2345-678", points:1680, referredBy:"m001", joinedAt:"2024-03-01", referralCode:"PNAIR-889",
    transactions:[{id:"t3a",pts:1680,icon:"⭐",label:"Welcome + Monthly Spend",date:"Mar 01",type:"earn"}]},
  { id:"m004", name:"Kevin Lim",    phone:"017-5544-332", points:210,  referredBy:"m002", joinedAt:"2024-04-05", referralCode:"KLIM-221",
...
export default function AdminApp() {
  ...
  const enrollMember = (name,phone,referredBy) => {
    const id=genId();
    const code=name.split(" ")[0].toUpperCase()+"-"+Math.floor(1000+Math.random()*9000);
    const newM={id,name,phone,points:100,referredBy:referredBy||null,joinedAt:new Date().toISOString().slice(0,10),referralCode:code,
...
function Enroll({ctx,onDone}){
  ...
  const submit=()=>{
    ...
    const m=enrollMember(form.name.trim(),form.phone,form.ref||null);
    showToast(`${m.name} enrolled! 100 welcome points awarded.`);
    setForm({name:"",phone:"",ref:""});setErr({});
  };
...
function Profile({ctx,memberId,onBack}){
  const {members,tiers,refLevels}=ctx;
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
      <div className="card" style={{padding:"28px 30px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {[{l:"Phone",v:member.phone},{l:"Member ID",v:member.id},{l:"Joined",v:member.joinedAt},{l:"Referral Code",v:member.referralCode||"—"},{l:"Referred By",v:referrer?referrer.name:"—"}].map(r=>(
