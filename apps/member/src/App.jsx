...
function LoginScreen({members,tiers,onLogin}){
  ...
  const login=()=>{
    ...
    const found=members.find(m=>m.phone.replace(/\D/g,"")===raw);
    if(!found){setErr("No member found with this number.");return;}
    onLogin(found.id);
  };
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",background:"linear-gradient(170deg,#1a1208 0%,#0d0a06 50%,#1a1208 100%)",position:"relative",overflow:"hidden"}}>
      ...
      <div className="si" style={{width:"100%",background:"#14100a",border:"1px solid #3a2a12",borderRadius:22,padding:"32px 28px",animationDelay:".12s"}}>
        ...
        <input value={phone} onChange={e=>{setPhone(fmtPhone(e.target.value));setErr("");}} onKeyDown={e=>e.key==="Enter"&&login()} placeholder="012-3456-789"
        ...
        <button onClick={login} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#f5c842,#f59e0b)",borderRadius:14,fontSize:15,fontWeight:700,color:"#1a1208",fontFamily:"'DM Sans',sans-serif",letterSpacing:.3,boxShadow:"0 4px 20px #f5c84244",border:"none",transition:"all .2s"}}>
          Access My Account →
        </button>
      </div>
